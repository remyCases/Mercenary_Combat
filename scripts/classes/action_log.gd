...
class ExecutionLog:
	...
	func execution_to_log(string_builder: String) -> String:
		...
		string_builder += "{name} acts first using {action}\n".format({
			"name": attacker_name,
			"action": action
		});
		...
		string_builder += "{name}'s attack is {outcome}\n".format({
			"name": attacker_name,
			"outcome": Enums.outcome_to_str(outcome),
		})
...
func log_to_str() -> String:
	...
	if first.action == "DEAD":
		string_builder += "{name}: Dead\n".format({
			"name": first.attacker_name,
		})
	else:
		string_builder += "{name}: {alertness}, {health}, {stance}\n".format({
			"name": first.attacker_name,
			"alertness": Enums.alertness_to_des(first.summary.attacker_alertness),
			"health": Enums.health_to_des(first.summary.attacker_health),
			"stance": Enums.stance_to_des(first.summary.attacker_stance),
		})
